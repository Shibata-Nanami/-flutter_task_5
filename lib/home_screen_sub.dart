...
//メモ一覧のデータ呼び出し
  ...
  Future<void> _fetchMemoData() async {
    await context.read<Memo>().fetchMemo();
  }
      ...
      body: SafeArea(
        child: SizedBox(
          ...
          child: Padding(
            ...
            child: context.watch<Memo>().isLoading
                ? const Center(
                    child: SizedBox(
                      height: 80,
                      width: 80,
                        ...
                        child: Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              Padding(
                                padding:
                                    const EdgeInsets.only(left: 30), // 左側に空白を追加
                                child: Text(modelData.title),
                              ),
                              const Spacer(), //2つのウィジェットの間隔を押し広げる
                              Padding(
                                padding:
                                    const EdgeInsets.only(right: 20), //右側に空白を追加
                                child: InkWell(
                                  child: IconButton(
...
//           //Memoデータを取得
//           future: context.read<Memo>().fetchMemo(),
//           //snapshotは非同期処理の結果を保持しており、その状態に応じて処理を分岐
//           builder: (BuildContext context, snapshot) {
//             print(snapshot);
//             //snapshotがデータを持っている場合（snapshot.hasDataがtrueの場合）"非同期処理中"というメッセージを出力
//             if (snapshot.hasData) {
//               print('非同期処理終了');
//               return SizedBox(
//                 height: MediaQuery.of(context).size.height,
